...
public partial class ADAM_AutoIO_LtoH_Latch_Form : Form, iATester.iCom
{
    ...
    /// main
    /// </summary>
    public ADAM_AutoIO_LtoH_Latch_Form()
    {
        InitializeComponent();
    }
    ...
    private void timer1_Tick(object sender, EventArgs e)
    {
        switch (mainTaskStp)
        {
            ...
            case (int)ADAM_AT_DiLtoHlatch_Task.wCh_Init:
                ProcessView(new ListViewData() { Ch = Ref_IO_Mod.Ch, Step = "Initialized", Result = "" });
                GetDeviceItems(di_id_offset + Ref_IO_Mod.Ch);
                ResetDefault();
                mainTaskStp++;
                break;
            case (int)ADAM_AT_DiLtoHlatch_Task.wCh_DiTask:
                //    l to h latch
                if (StpChkIdx1)
                {
                    ProcessStep = ADAM_AT_DiLtoHlatch_Task.wCh_DiTask.ToString();
            ...
            case (int)ADAM_AT_DiLtoHlatch_Task.wFinished:
                //    //列出上個步驟的結果
                ProcessStep = ADAM_AT_DiLtoHlatch_Task.wFinished.ToString();
    ...
    private void DoDiTask(int id)
    {
        ...
        else if (DiChLtoHStp > 7 && DiChLtoHStp < 99)
        {
        ...
        else if (DiChLtoHStp == 7) //clear modbus flag
        {
            ClrDiLatStatus();
            GetDeviceItems(id);
        ...
        else if (DiChLtoHStp == 6) //check modbus for Di L to H
        {
            if (ReadDiLatStatus())
                DiChLtoHStp++;
            else DiChLtoHStp = 99;
        ...
        else if (DiChLtoHStp == 5) //check Di status is High otherwise fail++
        {

            GetDeviceItems(id);
            if (!DoDiDetect(di_id_offset + Ref_IO_Mod.Ch)) DiChLtoHStp++;
            else DiChLtoHStp = 99;
        }
        else if (DiChLtoHStp == 4) //out apax5070 DO to L
        {
        ...
        else if (DiChLtoHStp == 3)//check Di status is High otherwise fail++
        {
            GetDeviceItems(id);
            if (!DoDiDetect(di_id_offset + Ref_IO_Mod.Ch)) DiChLtoHStp++;
            else DiChLtoHStp = 99;
        ...
        else if (DiChLtoHStp == 2)//out apax5070 DO to H
        {
        ...
        else if (DiChLtoHStp == 1)//set check Di status is low otherwise break
        {
            GetDeviceItems(id);
            //md 0=DI, md 1=counter, md 2=L to H ,md 3=H to L, md 4=f
            IOitem.Md = 2; //L to H mode
            ADAM6KReqService.UpdateIOConfig(IOitem);
            if (DoDiDetect(di_id_offset + Ref_IO_Mod.Ch)) DiChLtoHStp++;
            else DiChLtoHStp = 99;

        }
        else if (DiChLtoHStp == 0)//initial
        {
            ResStr01
              = MBData01 = "";
            ClrDiLatStatus();
    ...
    private bool ReadDiLatStatus()
    {
        ...
        ProcessView(new ListViewData()
        {
            Ch = Ref_IO_Mod.Ch,
            Step = DiChLtoHStp.ToString(),
            RowData = "Mbs LtoH 0x" + idx + " val = " + flg
        });
        MBData01 = ADAMmds.ReadCoils(idx, 1)[0].ToString();
        return flg;
    }
...
public enum ADAM_AT_DiLtoHlatch_Task
{
    wCh_Init = 1,
    wCh_DiTask= 2,

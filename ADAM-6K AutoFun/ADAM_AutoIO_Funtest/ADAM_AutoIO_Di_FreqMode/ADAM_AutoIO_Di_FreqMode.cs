...
public partial class ADAM_AutoIO_Di_FreqMode_Form : Form, iATester.iCom
{
    ...
    /// main
    /// </summary>
    public ADAM_AutoIO_Di_FreqMode_Form()
    {
        InitializeComponent();
    }

    private void ADAM_AutoIO_Di_FreqMode_Load(object sender, EventArgs e)
    {
        #region -- Item --
    ...
    private bool ProcessInitial()
    {
        ...
        {
            eLog(this, new LogEventArgs("ADAM_AutoIO_DI_FreqMode.exe", "Module is not support."));
    ...
    private void timer1_Tick(object sender, EventArgs e)
    {
        switch (mainTaskStp)
        {
            ...
            case (int)ADAM_AT_DiFqMode_Task.wCh_Init:
                ProcessView(new ListViewData() { Ch = Ref_IO_Mod.Ch, Step = "Initialized", Result = "" });
                GetDeviceItems(di_id_offset + Ref_IO_Mod.Ch);
                ResetDefault();
                mainTaskStp++;
                break;
            case (int)ADAM_AT_DiFqMode_Task.wCh_DiTask:
                //    l to h latch
                if (StpChkIdx1)
                {
                    ProcessStep = ADAM_AT_DiFqMode_Task.wCh_DiTask.ToString();
            ...
            case (int)ADAM_AT_DiFqMode_Task.wFinished:
                //    //列出上個步驟的結果
                ProcessStep = ADAM_AT_DiFqMode_Task.wFinished.ToString();
    ...
    private void DoDiTask(int id)
    {
        ...
        else if (DiCntMdStp == 3)//判斷Fq val Modbus status >=99 (4x01) 約99/10=9.9hz up 
        {
            ...
            if (ReadFreqVal(di_id_offset + Ref_IO_Mod.Ch) >= 99) DiCntMdStp++;
            else DiCntMdStp = 99;
        ...
        else if (DiCntMdStp == 1)//切換到Fq mode, chk cnt/fq =0 cntotherwise break
        {
            ...
            if (ReadClrCntVal(di_id_offset + Ref_IO_Mod.Ch)) DiCntMdStp++;
            else DiCntMdStp = 99;
    ...
    void  FrqOut() 
    {
        ...
        while(true)
        {
            a = 1 - a;
            OutputDO(a);
            //Thread.Sleep(88);  // out 5hz
            Thread.Sleep(40);  // out 10hz            
        }
    }
    ...
    private bool ReadClrCntVal(int _id)
    {
        ...
        GetDeviceItems(_id);
        if (ADAMmds.ReadHoldingRegs(idx,1)[0]==0) flg = true;
        else flg = false;
        ProcessView(new ListViewData()
        {
            Ch = Ref_IO_Mod.Ch,
    ...
    private int ReadFreqVal(int _id)
    {
        //bool flg = false;
        int idx = 1 + 2 * Ref_IO_Mod.Ch;
...
public enum ADAM_AT_DiFqMode_Task
{
    wCh_Init = 1,
    wCh_DiTask=2,

...
public partial class ADAM_AutoIO_Di_Value_Form : Form, iATester.iCom
{
    ...
    /// main
    /// </summary>
    public ADAM_AutoIO_Di_Value_Form()
    {
        InitializeComponent();
    }

    private void ADAM_AutoIO_Di_Value_Load(object sender, EventArgs e)
    {
        #region -- Item --
    ...
    public void StartTest()//iATester
    {
        if (ADAMConnection())
        {
            ...
            //20161109 add
            if (!FinishFlg)
                eLog(this, new LogEventArgs("ADAM_AutoIO_Di_Value.exe", "Process is not finish."));
        }
        else
            eResult(this, new ResultEventArgs(iResult.Fail));
        //
    ...
    private void DoDiTask(int id)
    {
        ...
        else if (DiChValStp > 7 && DiChValStp < 99)
        {
        ...
        else if (DiChValStp == 7) //判斷DI status Modbus status=0 (0x01)
        {
            //MBData01 = ReadDiVal(di_id_offset + Ref_IO_Mod.Ch).ToString();
            if (!ReadDiVal(di_id_offset + Ref_IO_Mod.Ch)) DiChValStp++;
            else DiChValStp = 99;
        }

        else if (DiChValStp == 6) //確認DI status是low
        {
            //val=0 is true, val=1 is false
            if (DoDiDetect(di_id_offset + Ref_IO_Mod.Ch)) DiChValStp++;
            else DiChValStp = 99;
        }
        else if (DiChValStp == 5) //out apax5070 DO to L
        {
        ...
        else if (DiChValStp == 4)//判斷DI status Modbus status=1 (0x01)
        {
            GetDeviceItems(id);
            //MBData01 = ReadDiVal(di_id_offset + Ref_IO_Mod.Ch).ToString();
            if (ReadDiVal(di_id_offset + Ref_IO_Mod.Ch)) DiChValStp++;
        ...
        else if (DiChValStp == 3)//check Di status is High otherwise fail++
        {
            GetDeviceItems(id);
            if (!DoDiDetect(di_id_offset + Ref_IO_Mod.Ch)) DiChValStp++;
            else DiChValStp = 99;
        ...
        else if (DiChValStp == 2)//out apax5070 DO to H
        {
        ...
        else if (DiChValStp == 1)//set check Di status is low otherwise break
        {
            GetDeviceItems(id);
    ...
    private bool ReadDiVal(int _id)
    {
        bool flg = false;
        int idx = 1 +  Ref_IO_Mod.Ch;
        GetDeviceItems(_id);
        ...
        ProcessView(new ListViewData()
        {
            Ch = Ref_IO_Mod.Ch,
            Step = DiChValStp.ToString(),
            RowData = "Mbs ch val = " + flg
            //+", cEn = " + Ref_IO_Mod.cEn.ToString()
        });
        MBData01 = flg.ToString();
        return flg;
    }

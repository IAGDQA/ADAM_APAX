...
public partial class ADAM_AutoIO_DigitalInOutputCmdSet_Form : Form, iATester.iCom
{
    ...
    /// main
    /// </summary>
    public ADAM_AutoIO_DigitalInOutputCmdSet_Form()
    {
        InitializeComponent();
    }

    private void ADAM_AutoIO_DigitalInOutputCmdSet_Load(object sender, EventArgs e)
    {
        #region -- Item --
    ...
    public void StartTest()//iATester
    {
        if (ADAMConnection())
        {
            ...
            //20161109 add
            if (!FinishFlg)
                eLog(this, new LogEventArgs("ADAM_DigitalInOutputCmdSet.exe", "Process is not finish."));
        }
        else
            eResult(this, new ResultEventArgs(iResult.Fail));
        //
    ...
    private void DoDiTask(int id)
    {
        ...
        else if (DiChValStp == 2)//SendCmd("$01F"); //Read DI value when setAllTrue
        {
            ...
            {
                case "Adam6050":
                    if (tmp1 == "!01" && tmp2 == "0FFF") ResStr02 = "Passed";
                ...
                case "Adam6051":
                    ...
                    break;
                case "Adam6052":
                    if (tmp1 == "!01" && tmp2 == "00FF") ResStr02 = "Passed";
                    else { ResStr02 = "Failed"; FailCnt++; }
                    break;
                case "Adam6060":
                    if (tmp1 == "!01" && tmp2 == "003F") ResStr02 = "Passed";
                    else { ResStr02 = "Failed"; FailCnt++; }
                    break;
                case "Adam6066":
                    if (tmp1 == "!01" && tmp2 == "003F") ResStr02 = "Passed";
    ...
    private int OutputDO(bool[] coil)
    {
        OutputAPAX5070(coil);
        return coil.Length;
    }
    ...
    private void OutputAPAX5070(bool[] coil)
    {
        int idx = OutDO_Slot_Idx; 
        //int idx = 1; //for debug
        APAX5070Service.ForceMultiCoils(idx, coil);
    }
    private void InputAPAX5070(uint coil)
    {
        //int idx = OutDI_Slot_Idx; 
        int idx = 65; //for debug
        APAX5070Service.ReadCoils(idx, (int)coil);
    }
    private int InputDi(uint coil)
    {
        InputAPAX5070(coil);
        return (int)coil;
    }
